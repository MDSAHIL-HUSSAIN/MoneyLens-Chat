...
const SourceNode = ({ data }) => {
  return (
    <div className="bg-white border-2 border-indigo-200 rounded-xl p-3 shadow-sm min-w-[160px] relative group transition-all hover:border-indigo-400 hover:shadow-md">
      {/* Connection Point */}
      ...
      {/* Hover Tooltip (Data details) */}
      <div className="absolute left-1/2 -translate-x-1/2 bottom-[105%] mb-2 opacity-0 group-hover:opacity-100 transition-opacity w-48 bg-gray-800 text-white text-xs p-2.5 rounded-lg shadow-xl pointer-events-none z-50">
...
const FinalNode = ({ data }) => {
  return (
    <div className="bg-gradient-to-br from-purple-500 to-indigo-600 border border-purple-400 rounded-xl p-4 shadow-lg min-w-[150px] relative group text-white">
      {/* Connection Point */}
...
export default function TrustGraph({ graphData }) {
  ...
  if (!graphData || !graphData.nodes) {
    return <div className="p-4 text-center text-gray-500">Generating trust logic...</div>;
  }

  return (
    <div className="w-full h-[300px] bg-gray-50/50 rounded-xl border border-gray-200 overflow-hidden">
      <ReactFlow
